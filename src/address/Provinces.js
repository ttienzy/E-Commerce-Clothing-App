...
import { UserAddress } from "../home/Register";
...
const Province = () => {
    const setAddress = useContext(UserAddress)
    const [selectedCode, setSelectedCode] = useState('')

    const handleProvinceChange = (event) => {
        setSelectedCode(event.target.value)    
        var selectName = event.target.options[event.target.selectedIndex].text;
        setAddress((preList) => ({...preList, province : selectName }))
    }
    return (
        <>
            <select 
                ...
                style = {{ height: '30px', padding : '4px', borderRadius : '4px', outlineColor : '#9d9dd7' }}>
            ...
            {(selectedCode && <Districts code={selectedCode} />) 
                || 
            <>
                <div>
                    <select className="districtList" disabled style={{ width:'100%', height: '30px', padding : '4px', borderRadius : '4px', outlineColor : '#9d9dd7'}}>
                        <option value="-1">Select your district</option>
                    </select>
                </div>
                <div>
                    <select className="wardList" disabled style={{ width:'100%', height: '30px', padding : '4px', borderRadius : '4px', outlineColor : '#9d9dd7'}}>
